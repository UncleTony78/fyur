...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:@localhost:5432/projectfyur'
...
def venues():
  ...
  start = datetime.now()
  data= []
  venue_info = db.session.query(Venue.city, Venue.state).distinct(Venue.city, Venue.state)
  for location in venue_info:
    num_shows = []
    for num in venues: 
      if (num.city==location[0]) and (num.state==location[1]):
        num_venue_shows = Show.query.filter_by(venue_id=num.id).all()
        num_upcoming_shows = 0
        for show in num_venue_shows:
          if show.start_time > start:
    ...
    venue_location = db.session.query(Venue.id, Venue.name).filter(Venue.city==location[0], Venue.state==location[1])
    data.append({
      'city': location[0],
      'state': location[1],
...
def search_venues():
  ...
  entry = request.form.get('search_term', '')
  venue_search = Venue.query.filter(Venue.name.ilike(f'%{entry}%')).all()
  data = []
  now = datetime.now()
...
def show_venue(venue_id):
  ...
  venue_show = Show.query.filter(Show.id==venue_id)
  now = datetime.now()
  Past_show = []
  Upcoming_show = []

  for item in venue_show:
    artist = Artist.query.filter_by(Artist.name,Artist.image_link)
    add_show = {
      'artist_id': item.artist_id,
      'artist_name': artist.name,
      'artist_image_link': artist.image_link,
      "start_time": format_datetime(str(item.start.time))
    }
    if item.start_time < now:
      Past_show.append(add_show)
    else:
        Upcoming_show.append(add_show)
...
def create_venue_submission():
  ...
  if not form.validate():
    ...
    any_error = False
    try:
      insert_new_venue = Venue(name=name, city=city, state=state, address=address, phone=phone, genres=genres,
      ...
      db.session.add(insert_new_venue)
      db.session.commit()
...
def delete_venue(venue_id):
  ...
  if not venue:
    ...
    try:  
      ...
      flash(f'Venue {name_of_venue} deleted')
      db.session.commit()
    except:
      delete_error = True
      db.session.rollback()
      ...
      db.session.close()
    if delete_error:
      flash(f'Error when deleting venue {name_of_venue}')
...
def search_artists():
  ...
  entry = request.form.get('search_term', '')
  artist_search = Artist.query.filter(Artist.name.ilike(f'%{entry}%')).all()
  data = []
  now = datetime.now()
...
def show_artist(artist_id):
  ...
  for item in artist.shows:
    if item.start_time > now:
      ...
      'venue_id': item.artist_id,
      'venue_name': item.name,
      'venue_image_link': item.image_link,
      "venue_time": format_datetime(str(item.start.time))
      })
    if item.start_time < now:
      past_show_count +=1
      Past_show.append({
...
def create_artist_submission():
  ...
  if not form.validate():
    ...
    any_error = False
    try:
      insert_new_artist = Artist(name=name, city=city, state=state, phone=phone, genres=genres,
      seeking_venue=seeking_venue, seeking_description=seeking_description, image_link=image_link,
      website=website, facebook_link=facebook_link)

      db.session.add(insert_new_artist)
      db.session.commit()
...
def create_show_submission():
  ...
  try:
    new_show_entry = Show(start_time=start_time, venue_id=venue_id, artist_id=artist_id)
